...
from ..database import get_db, DictionaryEntry
from ..models import WordEntry, WordResponse, MessageResponse
...
def create_entry(entry: WordEntry, db: Session = Depends(get_db)):
    ...
    if entry.word in dictionary.entries:
        raise HTTPException(status_code=400, detail=f"Entry for '{entry.word}' already exists")
